...
while ( $time <= $limit ) {    # check each second
    ...
    foreach my $deer (
        sort {
            $points{$b}->{distance} <=> $points{$a}->{distance}
        } keys %points )
    {
        $max = $points{$deer}->{distance}
            if $points{$deer}->{distance} > $max;
...
foreach my $category ( sort keys %categories ) {
    ...
    foreach my $deer (
        sort {
            $points{$b}->{$category} <=> $points{$a}->{$category}
        } keys %points )
    {
        printf( "\#%d   %4s%s%s\n",
                $rank, $deer,
